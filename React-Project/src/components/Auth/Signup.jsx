...
function Signup() {
    ...
    script.onload = () => {
      if (window.google) {
        window.google.accounts.id.initialize({
          ...
          client_id: process.env.REACT_APP_GOOGLE_CLIENT_ID, // You'll need to set this in your .env file
  ...
  const handleGoogleSignup = async (response) => {
    try {
      ...
      // 2. Extract user info and create account directly
      ...
      if (result.success) {
        window.location.href = '/dashboard'; // or wherever you want to redirect after successful signup
      } else {
  ...
  const handleGoogleButtonClick = () => {
    ...
    if (window.google) {
      window.google.accounts.id.prompt(); // Show the Google One Tap dialog
    } else {
      setError('Google OAuth is not loaded. Please refresh the page and try again.');
    }
  };
        ...
        className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400  transition-colors bg-transparent border-none outline-none focus:outline-none"
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="flex items-center justify-center px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white hover:bg-white/15 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isGoogleLoading ? (
